...
def _get_player(inter: nextcord.Interaction[Bot]) -> sonolink.Player | None:
    vc = inter.guild.voice_client if inter.guild else None
    return vc if isinstance(vc, sonolink.Player) else None


async def filter_autocomplete(inter: nextcord.Interaction[Bot], string: str) -> None:
    await inter.response.send_autocomplete(  # pyright: ignore[reportUnknownMemberType]
        [f.value for f in Filter if string.lower() in f.value.lower()][:25]
    )
...
async def apply_filter(
    ...
    reset: bool = nextcord.SlashOption(
        default=False,
...
@apply_filter.on_autocomplete("filter")
async def apply_filter_autocomplete(
    inter: nextcord.Interaction[Bot], filter: str
) -> None:
    await filter_autocomplete(inter, filter)
...
@remove_filter.on_autocomplete("filter")
async def remove_filter_autocomplete(
    inter: nextcord.Interaction[Bot], filter: str
) -> None:
    await filter_autocomplete(inter, filter)
...
async def reset_filters(inter: nextcord.Interaction[Bot]) -> None:
