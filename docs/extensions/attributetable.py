...
def visit_attributetable_node(self, node: attributetable) -> None:
    class_ = node["python-class"]
    self.body.append(f'<div class="py-attribute-table" data-move-to-id="{class_}">')


def visit_attributetablecolumn_node(self, node: attributetablecolumn) -> None:
    self.body.append(self.starttag(node, "div", CLASS="py-attribute-table-column"))


def visit_attributetabletitle_node(self, node: attributetabletitle) -> None:
    self.body.append(self.starttag(node, "span"))


def visit_attributetablebadge_node(self, node: attributetablebadge) -> None:
    attributes = {
...
def visit_attributetable_item_node(self, node: attributetable_item) -> None:
    self.body.append(self.starttag(node, "li", CLASS="py-attribute-table-entry"))


def depart_attributetable_node(self, node: attributetable) -> None:
    self.body.append("</div>")


def depart_attributetablecolumn_node(self, node: attributetablecolumn) -> None:
    self.body.append("</div>")


def depart_attributetabletitle_node(self, node: attributetabletitle) -> None:
    self.body.append("</span>")


def depart_attributetablebadge_node(self, node: attributetablebadge) -> None:
    self.body.append("</span>")


def depart_attributetable_item_node(self, node: attributetable_item) -> None:
    self.body.append("</li>")
...
def inject_attributetable(
    ...
    if getattr(options, "no_attributetable", None):
        return

    # `name` is the fully-qualified class name used by autodoc.
    lines[:0] = ["", f".. attributetable:: {name}", ""]


def setup(app: Sphinx):
    _OPTION_SPECS["class"] = _OPTION_SPECS["class"] | {"no-attributetable": bool_option}
    _OPTION_SPECS["exception"] = _OPTION_SPECS["exception"] | {
        "no-attributetable": bool_option
    }
    ...
    app.add_css_file("attributetable.css")
    app.connect("autodoc-process-docstring", inject_attributetable)
